...
public class RocketControl : MonoBehaviour
{
    ...
    public void BulletDamageCall()
    {
        ...
        {
            if (Vector3.Distance(transform.position, enemy.transform.position) <= smalestDistance)
            {
                closetEnemy = enemy;
                isCollided = true;
                smalestDistance = Vector3.Distance(transform.position, enemy.transform.position);
            }
        }
        ...
        if (isCollided && closetEnemy != null)
        {
            ...
            foreach (var enemy in spawner.GetComponent<EnemiesSpawner>().EnemyList)
            {
                if(Vector3.Distance(closetEnemy.transform.position,enemy.transform.position)<explosionRange)
                {
                    enemy.GetComponent<EnemyControl>().currentHitPoint -= BulletDamage;
                }
            }
        }
    }
